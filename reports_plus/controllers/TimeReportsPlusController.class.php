							...
							foreach ( $summarize_by_array as $current_level => $value ) {
								...
								if ($prev_value !== $row_value) {
									...
									foreach ( $final_headings as $key => $heading ) {
										...
										/***code added by jyoti**/
										/************Start of calculation of budget percentage(%)*********/
										       $active_project = Projects::findById ( $row ['project_id'] );
										       $budget = $active_project->getBudget();
                                              $cost_by_type = TrackingObjects::sumCostByTypeAndProject($this->logged_user, $active_project);if($cost_by_type) {
                                           $cost_so_far = 0;
										  ...
										  if($budget) {
											...
											$output_rows[$rowIdx]['budget'] = $budget ;
											$output_rows[$rowIdx]['cost_so_far_perc'] = $cost_so_far_perc ;
											 $output_rows[$rowIdx]['cost_over_budget_perc'] = $cost_over_budget_perc ;
											 /************End of calculation of budget percentage(%)*********/
