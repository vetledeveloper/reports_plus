<table class="global_time_record" id="global_time_report_records">
    ...
    {foreach from=$output_rows item=row}
            ...
            <tr class="heading{$row.current_level}">
                <td colspan="5">
    				...
    				{if $row.current_level != 0}
							{assign var="budget_color" value="#008000"}
							{if $row.estimate lt $row.hours}
								{assign var="budget_color" value="#FF0000"}
							{/if}
								<font style='color:{$budget_color}'>{$row.estimate}</font>
								/<font style='color:#2F4887;'>{$row.hours}</font>
					{/if}
    				{* code ended by jyoti *}
