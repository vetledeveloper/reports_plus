							...
							{assign var="over_str" value=0}
				             {if $row.current_level eq 0}
								 {* {if $row.budget > 0}*}
									 ...
									 {/if}
									 {if !($row.budget)}
									      <font style='color:#FF0000'>(There is no budget)</font>
									 {/if}
									 {if $over_str eq 1}
                                  <font style='color:#FF0000'>({$budget_perc}% over)</font>{/if}
								  {if $over_str eq 0 && ($row.budget)}
                                  <font style='color:#A5BC4E'>({$budget_perc}%)</font>{/if}
									 {/if}
                                  {*{/if}*}
				   {/if}
				   {*{/if}*}
